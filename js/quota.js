...
document.getElementById('donate-quota').addEventListener('click',function(event){
    ...
    const donation_balance=parseFloat(getInputFieldValueById('input-amount-quota'));
    const total_balance=parseFloat(getAttributeValueById('balance-quota'));
    ...
    setAttributeValueById('balance-quota',total_balance+donation_balance);
    setAttributeValueById('balance-main',main_balance-donation_balance);

    //add to history
    const head=getAttributeValueById('donate-quota-heading');
